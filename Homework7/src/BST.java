...
public class BST {
	...
	public void addLeftSubTreeToMin(BST subtree) {
		...
		} else if (((TreeIterator) findMax(subtree.root)).currentNode.data > ((TreeIterator) findMin(this.root)).currentNode.data) {
			return;
		} else {
			TreeIterator temp = (TreeIterator) findMin(this.root);
			temp.currentNode.left = subtree.root;
			subtree.root.parent = temp.currentNode;
			this.size += subtree.size;
		}
